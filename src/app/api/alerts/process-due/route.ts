...
export async function POST(request: Request) {
  ...
  if (errAlerts) {
    Sentry.captureException(errAlerts, {
      tags: { route: 'alerts/process-due', phase: 'fetch_alerts' },
    });
    return NextResponse.json({ error: 'Failed to fetch due alerts' }, { status: 500 });
  }
  ...
  for (const alert of dueAlerts) {
    ...
    try {
      ...
      Sentry.captureException(e, {
        tags: { route: 'alerts/process-due', alert_id: alert.id },
        extra: { runError },
      });
    }
