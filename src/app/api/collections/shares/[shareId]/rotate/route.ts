...
export async function POST(
  ...
  try {
    ...
    const key = getRateLimitKey('collections:shares:rotate', _request, user.id);
    ...
    const { data: share } = await supabase
      ...
      .select('id, org_id, collection_id, expires_at')
      .eq('id', shareId)
    ...
    const collectionId = (share as { collection_id: string }).collection_id;
    if (!collectionId) return NextResponse.json({ error: 'Invalid share' }, { status: 400 });
