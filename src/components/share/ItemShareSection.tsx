...
export function ItemShareSection({ itemId }: { itemId: string }) {
  ...
  async function fetchShares() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/items/${itemId}/shares`);
      if (res.ok) {
  ...
  async function handleCopy(share: Share) {
    ...
    const url = `${typeof window !== 'undefined' ? window.location.origin : ''}/share-item/${share.token}`;
  ...
  async function handleRotate(share: Share) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/items/shares/${share.id}/rotate`, {
        method: 'POST',
      });
      const data = await res.json().catch(() => ({}));
      if (res.ok) {
  ...
  async function handleRevoke(share: Share) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/items/shares/${share.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="space-y-3 rounded-lg border border-neutral-200 p-4 dark:border-neutral-700">
      ...
      {loading ? (
        ...
        <p className="text-sm text-neutral-500">No share links yet. Create one to share this item publicly.</p>
      ) : (
            ...
            return (
              ...
              >
                <div className="min-w-0 flex-1 text-sm">
                  <span className="block truncate font-mono text-xs" title={share.token}>
